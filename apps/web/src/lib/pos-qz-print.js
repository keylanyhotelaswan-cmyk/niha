import { readSavedPrinterName, savePrinterName } from './pos-receipt.js';
...
export function isQzMarkedAllowed() {
    try {
...
async function discoverPrinters(qz) {
    const found = new Set();
    const add = (names) => {
        if (!names)
            return;
...
export async function resolveQzPrinter() {
    const qz = await loadQz();
    if (!qz)
        return null;
...
async function runPrintStrategy(qz, printer, label, run) {
    try {
        await run();
...
export async function printReceiptToQz(printer, payload) {
    const qz = await loadQz();
    if (!qz)
...
export async function printHtmlToQz(printer, html) {
    await printReceiptToQz(printer, { html });
}
export async function silentPrintReceipt(payload) {
    return silentPrintReceipts([payload]);
}
export async function silentPrintReceipts(payloads) {
    if (!payloads.length)
        return { ok: false, reason: 'print-failed', message: 'لا يوجد ما يُطبع' };
    const qz = await loadQz();
    if (!qz) {
        return {
            ok: false,
...
export async function silentPrintHtml(html) {
