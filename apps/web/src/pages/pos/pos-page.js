...
import { Alert, Box, Button, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Fab, MenuItem, Paper, Snackbar, Stack, TextField, Typography, useMediaQuery, useTheme, } from '@mui/material';
...
export function PosPage() {
    ...
    const shiftSummaryPreviewParams = workspace.shiftOpen && workspace.posSummary ? {
    ...
    const handleAmendOrder = async (payload) => {
    ...
    const runCollect = async (withPrint) => {
        ...
        try {
            ...
            if (!res.ok) {
                setCollectError(parseApiErrorBody(res.body, res.error ?? 'فشل التحصيل'));
                notify(parseApiErrorBody(res.body, res.error ?? 'فشل التحصيل'));
                return;
            }
    ...
    return (_jsxs(Stack, { spacing: 2.5, children: [_jsx(Snackbar, { open: Boolean(snack), autoHideDuration: 4000, onClose: () => setSnack(''), anchorOrigin: { vertical: 'bottom', horizontal: 'center' }, children: _jsx(Alert, { severity: "success", variant: "filled", onClose: () => setSnack(''), sx: { width: '100%' }, children: snack }) }), shiftKnown && !workspace.shiftOpen && workspace.contextReady ? (_jsx(Alert, { severity: "warning", sx: { borderRadius: 3 }, action: _jsx(Button, { color: "inherit", size: "small", onClick: () => setShiftOpenDialog(true), children: "\u0641\u062A\u062D \u0648\u0631\u062F\u064A\u0629" }), children: "\u0627\u0644\u0648\u0631\u062F\u064A\u0629 \u0645\u063A\u0644\u0642\u0629 \u2014 \u0644\u0627 \u064A\u0645\u0643\u0646 \u062A\u0633\u062C\u064A\u0644 \u0637\u0644\u0628\u0627\u062A \u0623\u0648 \u0645\u0635\u0631\u0648\u0641\u0627\u062A \u062D\u062A\u0649 \u062A\u0641\u062A\u062D \u0627\u0644\u0648\u0631\u062F\u064A\u0629." })) : null, workspace.posContextError ? (_jsx(Alert, { severity: "error", sx: { borderRadius: 3 }, action: _jsx(Button, { size: "small", onClick: () => workspace.refetchPosContext(), children: "\u0625\u0639\u0627\u062F\u0629 \u0627\u0644\u0645\u062D\u0627\u0648\u0644\u0629" }), children: workspace.posContextErrorDetail?.message ?? 'فشل تحميل سياق نقطة البيع' })) : null, _jsxs(Paper, { elevation: 0, sx: {
                    ...
                    return { ok: true };
                }, onCloseOrder: () => { void order.closeOrder(); }, onClearCart: () => { order.resetOrder(); notify('تم إفراغ السلة.'); } }), auditOrder ? (_jsx(OrderAuditDialog, { open: Boolean(auditOrder), orderId: auditOrder.id, orderCode: auditOrder.code, onClose: () => setAuditOrder(null) })) : null, _jsx(OrderSummaryDialog, { open: Boolean(summaryOrder), order: summaryOrder, ...(() => {
                    if (!summaryOrder)
                        return {};
                    const label = catalog.paymentMethods.find((m) => m.id === summaryOrder.paymentMethod)?.label;
                    return label ? { paymentMethodLabel: label } : {};
                })(), onClose: () => setSummaryOrder(null) }), _jsx(OrderEditDialog, { open: Boolean(editOrder), branchId: workspace.effectiveBranchId, order: editOrder, products: catalog.products, sauces: catalog.sauces, deliveryDrivers: deliveryDrivers, onClose: () => setEditOrder(null), onSave: handleAmendOrder }), _jsxs(Dialog, { open: collectOpen, onClose: collectBusy ? undefined : () => { setCollectOpen(false); setCollectError(''); }, fullWidth: true, maxWidth: "xs", children: [_jsx(DialogTitle, { children: "\u062A\u0633\u062C\u064A\u0644 \u062A\u062D\u0635\u064A\u0644 \u0641\u064A \u0627\u0644\u062F\u0631\u062C" }), _jsx(DialogContent, { children: _jsxs(Stack, { spacing: 2, sx: { pt: 1 }, children: [collectError ? _jsx(Alert, { severity: "error", children: collectError }) : null, _jsx(TextField, { label: "\u0631\u0642\u0645 \u0627\u0644\u0637\u0644\u0628", value: collectOrder?.code ?? '', InputProps: { readOnly: true } }), _jsx(TextField, { label: "\u0627\u0644\u0625\u062C\u0645\u0627\u0644\u064A", value: collectOrder ? formatCurrency(collectOrder.total) : '', InputProps: { readOnly: true } }), _jsx(TextField, { select: true, label: "\u0637\u0631\u064A\u0642\u0629 \u0627\u0644\u062F\u0641\u0639", value: collectPayment, onChange: (e) => setCollectPayment(e.target.value), disabled: collectBusy, children: catalog.paymentMethods.map((m) => _jsx(MenuItem, { value: m.id, children: m.label }, m.id)) })] }) }), _jsxs(DialogActions, { sx: { flexWrap: 'wrap', gap: 1, px: 2, pb: 2 }, children: [_jsx(Button, { onClick: () => { setCollectOpen(false); setCollectError(''); }, disabled: collectBusy, children: "\u0625\u0644\u063A\u0627\u0621" }), _jsx(Button, { variant: "outlined", disabled: collectBusy, ...(collectBusy ? { startIcon: _jsx(CircularProgress, { size: 16 }) } : {}), onClick: () => void runCollect(false), children: collectBusy ? 'جاري التحصيل…' : 'تحصيل فقط' }), canUsePrint ? (_jsx(Button, { variant: "contained", disabled: collectBusy, ...(collectBusy ? { startIcon: _jsx(CircularProgress, { size: 16, color: "inherit" }) } : {}), onClick: () => void runCollect(true), children: collectBusy ? 'جاري التحصيل…' : 'تحصيل وطباعة' })) : null] })] }), canUsePrint ? (_jsx(PrintSetupDialog, { open: printSetupOpen, onClose: () => setPrintSetupOpen(false) })) : null, _jsxs(Dialog, { open: expenseOpen, onClose: () => setExpenseOpen(false), fullWidth: true, maxWidth: "sm", children: [_jsx(DialogTitle, { children: "\u062A\u0633\u062C\u064A\u0644 \u0645\u0635\u0631\u0648\u0641 \u0645\u0646 \u0627\u0644\u0648\u0631\u062F\u064A\u0629" }), _jsx(DialogContent, { children: _jsxs(Stack, { spacing: 2, sx: { pt: 1 }, children: [_jsx(Alert, { severity: "info", sx: { borderRadius: 2 }, children: "\u064A\u064F\u062E\u0635\u0645 \u0645\u0646 \u0639\u0647\u062F\u0629 \u0627\u0644\u0643\u0627\u0634\u064A\u0631 (\u0627\u0644\u062F\u0631\u062C) \u0648\u0644\u064A\u0633 \u0645\u0646 \u062E\u0632\u0646\u0629 \u0627\u0644\u0625\u062F\u0627\u0631\u0629." }), _jsxs(TextField, { select: true, label: "\u0646\u0648\u0639 \u0627\u0644\u0645\u0635\u0631\u0648\u0641", value: expenseKind, onChange: (e) => setExpenseKind(e.target.value), children: [_jsx(MenuItem, { value: "GENERAL", children: "\u0645\u0635\u0631\u0648\u0641 \u0639\u0627\u0645" }), _jsx(MenuItem, { value: "ITEM", children: "\u0634\u0631\u0627\u0621 \u062E\u0627\u0645\u0627\u062A" })] }), expenseKind === 'ITEM' ? (_jsxs(_Fragment, { children: [_jsx(TextField, { select: true, label: "\u0627\u0644\u0635\u0646\u0641", value: expenseStockItemId, onChange: (e) => {
