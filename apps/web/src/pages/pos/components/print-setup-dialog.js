...
import { bridgePrintJobs, isPrintBridgeOnline, listBridgePrinters, } from '../../../lib/pos-print-bridge.js';
export function PrintSetupDialog({ open, onClose }) {
    ...
    const refresh = async () => {
        ...
        if (list.includes(saved))
            ...
            setSelected(list.find((p) => /xp-80/i.test(p)) ?? list[0] ?? '');
        setStatus('ready');
    };
    useEffect(() => {
        if (open)
